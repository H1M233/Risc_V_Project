...
module bpu_controller #(
    ...
    wire bpu_query_flush = pred_mispredict | pred_taken;
    ...
    always@(posedge clk) begin
        if(!rst) begin
            ...
            // Only flush valid/control bits. Once the control is invalidated,
            // the wide data registers become don't-care for this cycle.
            gshare_prev_b       <= bpu_query_flush ? 1'b0 : is_B_type;
            pred_next_pc        <= pc_add_imm;
            gshare_pht_index    <= pht_index;
            prev_JALR           <= bpu_query_flush ? 1'b0 : is_JALR;
            prev_JALR_ret       <= bpu_query_flush ? 1'b0 : is_ras_pop;
            ras_pop_en          <= bpu_query_flush ? 1'b0 : is_ras_pop;
            btb_query_pc        <= pc_addr;
            prev_JAL            <= bpu_query_flush ? 1'b0 : is_JAL;
            ras_push_en         <= bpu_query_flush ? 1'b0 : is_ras_push;
