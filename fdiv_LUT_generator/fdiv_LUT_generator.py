...
class PTable:
    ...
    def recipmidP(self, x):
        return recipmid(x, self.inWidth, self.outWidth)

    def firstextend(self, xh):
        a = bitsConnect(xh, 0, 0)
        b = bitsConnect(xh, 0, XL_MAX)
        return self.recipmidP(a) - self.recipmidP(b)
    
    def lastextend(self, xh):
        a = bitsConnect(xh, XM_MAX, 0)
        b = bitsConnect(xh, XM_MAX, XL_MAX)
        return self.recipmidP(a) - self.recipmidP(b)
    
    def avgextend(self, xh):
        return (self.firstextend(xh) + self.lastextend(xh)) / 2.0
    
    def extend(self, xh, xm):
        a = bitsConnect(xh, xm, 0)
        b = bitsConnect(xh, xm, XL_MAX)
        return self.recipmidP(a) - self.recipmidP(b)

    def adjust(self, xh, xm):
        return (self.avgextend(xh) + self.extend(xh, xm)) / 2.0
    ...
    def generate(self):
        ...
        for xh in range(0, XH_MAX + 1):
            for xl in range(0, XL_MAX + 1):
                a = bitsConnect(xh, 0, xl)
                b = bitsConnect(xh, XM_MAX, xl)
                value = self.recipmidN(a) - 2
...
def GoldschmidtIteration(A, B, APPROX, k, quiet=False): # A / B
    # Iteration -1
    D = B
    F = APPROX
    N = A
...
def getApproxRecipB(B, quiet=False):
    P = PTable()
    N = NTable()
    PT = P.generate()
    NT = N.generate()
    ...
    resb = PQuery - NQuery
    res = resb / (2**16)
    delta = abs(1/B - res) / (1/B) * 100
    deltaWithoutN = abs(1/B - PQuery / (2**16)) / (1/B) * 100
...
def testGoldschmidtIteration(A, B):
    APPROX = getApproxRecipB(B)
    IterationResult = GoldschmidtIteration(A, B, APPROX, 100)
    print(f"IterationResult: {IterationResult}")
    return IterationResult


def testWorstIterationRounds():
    P = PTable()
    N = NTable()
    PT = P.generate()
    NT = N.generate()
                ...
                for AXh in range(0, XH_MAX + 1):
                    for AXm in range(0, XM_MAX + 1):
                        for AXl in range(0, XL_MAX + 1):
                            ...
                            IterationResult, IterationRounds = GoldschmidtIteration(A, B, ApproxRecipResult, 100, quiet=True)
...
def main():
    # testWorstIterationRounds()
    a = hex2real(0x40490fdb)
    b = hex2real(0x3f800000)
    print(a/b)
    res, k = testGoldschmidtIteration(a, b)
    getResMant2hex(res)
