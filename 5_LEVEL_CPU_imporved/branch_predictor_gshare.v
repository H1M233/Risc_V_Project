...
module branch_predictor_gshare #(
    ...
    // to if
    // output     [BHR_WIDTH-1:0] pred_index, // 本次预测使用的 PHT 索引（供流水线透传）
    ...
    wire    [BHR_WIDTH - 1:0]   pht_index           = pc_addr[BHR_WIDTH + 1:2] ^ bhr;
    wire    [BHR_WIDTH - 1:0]   update_pht_index    = update_pc[BHR_WIDTH + 1:2] ^ bhr;
    // assign  pred_index = pht_index;

    wire    is_b_type = (pc_inst[6:0] == `TYPE_B);
    wire    [31:0] b_imm = {{20{pc_inst[31]}}, pc_inst[7], pc_inst[30:25], pc_inst[11:8], 1'b0};    // 指令B的立即数
