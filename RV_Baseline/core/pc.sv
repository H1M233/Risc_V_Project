`include "rv32I.svh"
`include "switch.svh"

module pc(
    input  logic        clk                     ,
    input  logic        rst                     ,
    input  logic        pipe_hold               ,

    // from IAB
    input  logic [1:0]  IAB_update_offest       ,
    input  logic        IAB_prefetch            ,

    // flush
    input  flush_t      BPU_pred_flush          ,
    input  flush_t      EX_mispred_flush        ,
    input  flush_t      CSR_trap_flush          ,

    // to I-Cache
    output logic [31:0] pc_o                    ,
    output logic [31:0] ICACHE_req_pc_o         
);
    wire [31:0] pc_add_2 = pc_o + 32'd2;
    wire [31:0] pc_add_4 = pc_o + 32'd4;

    // 为冲刷 / 异常留的口
    logic        pc_flush_en;
    logic [31:0] pc_flush_sel;
    assign pc_flush_en = CSR_trap_flush.en | EX_mispred_flush.en | BPU_pred_flush.en;
    always_comb begin
        if (CSR_trap_flush.en) begin
            pc_flush_sel = CSR_trap_flush.pc;
        end else if (EX_mispred_flush.en) begin
            pc_flush_sel = EX_mispred_flush.pc;
        end else if (BPU_pred_flush.en) begin
            pc_flush_sel = BPU_pred_flush.pc;
        end else begin
            pc_flush_sel = 32'b0;
        end
    end

    always_ff @(posedge clk) begin
        if (rst) begin
            pc_o  <= `IROM_ADDR_START;
        end else if (pc_flush_en) begin
            pc_o <= pc_flush_sel;
        end else if (pipe_hold) begin
            // ...
        end else begin
            case (IAB_update_offest)
                2'b00   : pc_o <= pc_o;
                2'b01   : pc_o <= pc_add_2;
                2'b11   : pc_o <= pc_add_4;
                default : pc_o <= pc_o;
            endcase
        end
    end

    // I-Cache 基址
    always_ff @(posedge clk) begin
        if (rst) begin
            ICACHE_req_pc_o  <= `IROM_ADDR_START;
        end else if (pc_flush_en) begin
            ICACHE_req_pc_o  <= {pc_flush_sel[31:5], 5'b0};
        end else if (pipe_hold) begin
            // ...
        end else if (IAB_prefetch) begin
            ICACHE_req_pc_o  <= {pc_o[31:5], 5'b0} + 32'b100000;
        end
    end
endmodule